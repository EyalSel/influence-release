...
class GenericNeuralNet(object):
	...
	def __init__(self, **kwargs):
			...
			self.hvp_op = self.hessian_vector_product_op(self.total_loss, self.params, self.v_placeholder)
	...
	def adversarial_loss(self, logits, labels):
			...
			print('wrong_labels_bool', tf.shape(wrong_labels_bool))
	...
	def get_grad_of_influence_wrt_input(self, train_indices, test_indices,
			...
			logger.info("perturbation shape: {}, perturbation: {}".format(grad_influence_wrt_input_val.shape, grad_influence_wrt_input_val))
	...
	def grad_influence_wrt_input(aself, inverse_hvp, xTr, yTr):
	...
	def get_preds(self, test_data, test_indices):
			return self.sess.run(self.preds, feed_dict = self.fill_feed_dict_with_some_ex(test_data, test_indices))
